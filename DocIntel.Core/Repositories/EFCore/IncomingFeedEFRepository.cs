...
{
    /// <summary>
    ///     Provides an implementation of <see cref="IIncomingFeedRepository" />
    ...
    /// </summary>
    public class IncomingFeedEFRepository : DefaultEFRepository<Importer>, IIncomingFeedRepository
    {
        /// <summary>
        ...
        ///     addition is announced with a <see cref="IncomingFeedCreatedMessage" />
        ///     on the bus.
        /// </summary>
        ...
        ///     data annotations.
        /// </exception>
        public async Task<Importer> CreateAsync(AmbientContext ambientContext,
            Importer feed)
        {
            if (!await _appAuthorizationService.CanCreateIncomingFeed(ambientContext.Claims, feed))
            {
            ...
            if (IsValid(feed, modelErrors))
            {
                ...
                PublishMessage(ambientContext, new IncomingFeedCreatedMessage
                {
                    IncomingFeedId = trackingEntity.Entity.ImporterId
                });
        ...
        {
            var feed = await ambientContext.DatabaseContext.IncomingFeeds.FindAsync(feedId);
            if (feed != null) return await _appAuthorizationService.CanViewIncomingFeed(ambientContext.Claims, feed);
        ...
        /// </exception>
        public async Task<Importer> GetAsync(AmbientContext ambientContext, Guid feedId,
            Func<IQueryable<Importer>,IQueryable<Importer>> includes = null)
        {
            IQueryable<Importer> enumerable = ambientContext.DatabaseContext.IncomingFeeds;

            if (includes != null)
                enumerable = includes(enumerable);

            var feed = enumerable.SingleOrDefault(_ => _.ImporterId == feedId);

            if (feed == null)
                throw new NotFoundEntityException();

            if (await _appAuthorizationService.CanViewIncomingFeed(ambientContext.Claims, feed))
                return feed;
            throw new UnauthorizedOperationException();
        }
        ...
        /// </returns>
        public async IAsyncEnumerable<Importer> GetAllAsync(AmbientContext ambientContext,
        ...
        /// <summary>
        ///     Removes an existing incoming feed from the database. The
        ///     removal is announced with a <see cref="IncomingFeedRemovedMessage" />
        ///     on the bus.
        /// </summary>
        /// <param name="feedId">
        ...
        ///     incoming feed from the database.
        /// </exception>
        /// <exception cref="NotFoundEntityException">
        ///     Thrown when the specified feed is not known in the database.
        /// </exception>
        public async Task<Importer> RemoveAsync(AmbientContext ambientContext,
            Guid feedId)
        {
            var feed = await ambientContext.DatabaseContext.IncomingFeeds.FindAsync(feedId);
            if (feed == null)
            ...
            if (!await _appAuthorizationService.CanDeleteIncomingFeed(ambientContext.Claims, feed))
                throw new UnauthorizedOperationException();

            var trackingEntity = ambientContext.DatabaseContext.Remove(feed);
            PublishMessage(ambientContext, new IncomingFeedRemovedMessage
            {
                IncomingFeedId = trackingEntity.Entity.ImporterId
            });
        ...
        /// <summary>
        ///     Updates an existing incoming feed in the database. The
        ///     removal is announced with a <see cref="IncomingFeedUpdatedMessage" />
        ///     on the bus.
        /// </summary>
        ...
        ///     Thrown when the specified feed is not known in the database.
        /// </exception>
        public async Task<Importer> UpdateAsync(AmbientContext ambientContext,
            Importer feed)
        {
            var retrievedFeed = await ambientContext.DatabaseContext.IncomingFeeds.FindAsync(feed.ImporterId);
            if (retrievedFeed == null)
                throw new NotFoundEntityException();

            if (!await _appAuthorizationService.CanEditIncomingFeed(ambientContext.Claims, retrievedFeed))
                throw new UnauthorizedOperationException();
            ...
            if (IsValid(feed, modelErrors))
            {
                ...
                PublishMessage(ambientContext, new IncomingFeedUpdatedMessage
                {
                    IncomingFeedId = trackingEntity.Entity.ImporterId
                });
                return trackingEntity.Entity;
        ...
        private static IQueryable<Importer> BuildQuery(IQueryable<Importer> incomingFeeds,
            int page, int limit)
        {
            if ((page > 0) & (limit > 0))
                incomingFeeds = incomingFeeds.Skip((page - 1) * limit).Take(limit);

            return incomingFeeds;
        }
    }
}