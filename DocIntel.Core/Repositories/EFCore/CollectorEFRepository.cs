...
{
    /// <summary>
    ///     Provides an implementation of <see cref="ICollectorRepository" />
    ...
    /// </summary>
    public class CollectorEFRepository : DefaultEFRepository<Collector>, ICollectorRepository
    {
        /// <summary>
        ///     Initialize a new instance of <see cref="CollectorEFRepository" />.
        /// </summary>
        ...
        public CollectorEFRepository(IPublishEndpoint busClient,
            IAppAuthorizationService appAuthorizationService)
        : base(context => context.DatabaseContext.Collectors,
        ...
        ///     addition is announced with a <see cref="CollectorCreatedMessage" />
        ///     on the bus.
        /// </summary>
        ...
        ///     collector in the repository.
        /// </exception>
        /// <exception cref="InvalidArgumentException">
        ///     Thrown when the specified feed is not valid according to its
        ///     data annotations.
        /// </exception>
        public async Task<Collector> CreateAsync(AmbientContext ambientContext,
            Collector feed)
        {
            if (!await _appAuthorizationService.CanCreateCollector(ambientContext.Claims, feed))
            {
            ...
            if (IsValid(feed, modelErrors))
            {
                ...
                PublishMessage(ambientContext, new CollectorCreatedMessage
                {
                    CollectorId = trackingEntity.Entity.CollectorId
                });
        ...
        {
            var feed = await ambientContext.DatabaseContext.Collectors.FindAsync(feedId);
            if (feed != null) return await _appAuthorizationService.CanViewCollector(ambientContext.Claims, feed);
        ...
        /// </exception>
        public async Task<Collector> GetAsync(AmbientContext ambientContext, Guid feedId,
            Func<IQueryable<Collector>,IQueryable<Collector>> includes = null)
        {
            IQueryable<Collector> enumerable = ambientContext.DatabaseContext.Collectors;

            if (includes != null)
                enumerable = includes(enumerable);

            var feed = enumerable.SingleOrDefault(_ => _.CollectorId == feedId);

            if (feed == null)
                throw new NotFoundEntityException();

            if (await _appAuthorizationService.CanViewCollector(ambientContext.Claims, feed))
                return feed;
            throw new UnauthorizedOperationException();
        }
        ...
        /// </returns>
        public async IAsyncEnumerable<Collector> GetAllAsync(AmbientContext ambientContext,
            Func<IQueryable<Collector>,IQueryable<Collector>> query = null,
            Func<IQueryable<Collector>,IQueryable<Collector>> includes = null,
        ...
        /// <summary>
        ///     Removes an existing collector from the database. The
        ///     removal is announced with a <see cref="CollectorRemovedMessage" />
        ///     on the bus.
        /// </summary>
        /// <param name="feedId">
        ...
        ///     collector from the database.
        /// </exception>
        /// <exception cref="NotFoundEntityException">
        ///     Thrown when the specified feed is not known in the database.
        /// </exception>
        public async Task<Collector> RemoveAsync(AmbientContext ambientContext,
            Guid feedId)
        {
            var feed = await ambientContext.DatabaseContext.Collectors.FindAsync(feedId);
            if (feed == null)
            ...
            if (!await _appAuthorizationService.CanDeleteCollector(ambientContext.Claims, feed))
                throw new UnauthorizedOperationException();

            var trackingEntity = ambientContext.DatabaseContext.Remove(feed);
            PublishMessage(ambientContext, new CollectorRemovedMessage
            {
                CollectorId = trackingEntity.Entity.CollectorId
            });
        ...
        /// <summary>
        ///     Updates an existing collector in the database. The
        ///     removal is announced with a <see cref="CollectorUpdatedMessage" />
        ///     on the bus.
        /// </summary>
        ...
        ///     Thrown when the specified feed is not known in the database.
        /// </exception>
        public async Task<Collector> UpdateAsync(AmbientContext ambientContext,
            Collector feed)
        {
            var retrievedFeed = await ambientContext.DatabaseContext.Collectors.FindAsync(feed.CollectorId);
            if (retrievedFeed == null)
                throw new NotFoundEntityException();

            if (!await _appAuthorizationService.CanEditCollector(ambientContext.Claims, retrievedFeed))
                throw new UnauthorizedOperationException();
            ...
            if (IsValid(feed, modelErrors))
            {
                ...
                PublishMessage(ambientContext, new CollectorUpdatedMessage
                {
                    CollectorId = trackingEntity.Entity.CollectorId
                });
                return trackingEntity.Entity;
        ...
        private static IQueryable<Collector> BuildQuery(IQueryable<Collector> collectors,
            int page, int limit)
        {
            if ((page > 0) & (limit > 0))
                collectors = collectors.Skip((page - 1) * limit).Take(limit);
